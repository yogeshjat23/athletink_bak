...
app.use(express.static(path.join(__dirname1,"AthleteLink_frontend" , "build"))); // Serve static files from the React build folder

// Fallback to React's index.html for all other routes (important for client-side routing)
app.get("*", (req, res) => {
  res.sendFile(path.join(__dirname1,"AthleteLink_frontend"  ,"build", "index.html"));
});
